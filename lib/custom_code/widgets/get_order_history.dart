...
class GetOrderHistory extends StatefulWidget {
  final double? height;
  final double? width;
  final DocumentReference? dr;
  const GetOrderHistory({Key? key, this.height, this.width, this.dr})
      : super(key: key);

  @override
  State<GetOrderHistory> createState() => _GetOrderHistoryState();
}

class _GetOrderHistoryState extends State<GetOrderHistory> {
  @override
  void initState() {
    super.initState();
    getOrder(widget.dr);
      ...
      if (value.docs.length > 0) {
        for (int i = 0; i < value.docs.length; i++) {
          if (!count.contains(value.docs.elementAt(i).get("order_number"))) {
            ...
            m[o.indexOf(value.docs.elementAt(i).get("order_number"))][4] =
                m[o.indexOf(value.docs.elementAt(i).get("order_number"))][4] +
                    1;
          }
        }
